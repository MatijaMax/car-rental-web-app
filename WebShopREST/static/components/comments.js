...
<div id="all-comment-div">
	...
	<div id="comment-div" v-for="c in allComments">
		...
		<button v-if="currentRole === 'MANAGER' && c.status === 'waiting'" class="btn btn-success" @click="setCommentStatus('approved', c.id)">Approve</button>
		<button v-if="currentRole === 'MANAGER' && c.status === 'waiting'" class="btn btn-danger" @click="setCommentStatus('rejected', c.id)">Reject</button>
...
`
	...
	mounted () {
		...
		if(this.currentRole === 'CUSTOMER'  || this.currentRole === '')
