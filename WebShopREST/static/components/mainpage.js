Vue.component("Mainpage", {
	data: function () {
		    return {
				...
				sortRating: 0,
				isOpenChecked: false,
				types: ["CAR", "VAN", "MOBILEHOME"],
				ftypes: ["DIESEL", "PETROL", "HYBRID", "ELECTRIC"]
		    }
    ...
    <div id="mainpage-div">
        <div id="signin-buttons">
            ...
            <button v-if = "this.role === 'MANAGER'" v-on:click = "createSportsFacility" class="btn btn-info"> Create facility </button>
            <button v-if = "this.role === 'ADMIN'" v-on:click = "showRegisterUser" class="btn btn-info"> Register new manager </button>
			<button v-if = "this.role === 'ADMIN'" v-on:click = "createSportsFacility" class="btn btn-info"> Create car rental facility </button>
            <button v-if = "this.role === 'ADMIN'" v-on:click = "showAllUsers" class="btn btn-info"> View all users </button>
            
            <button v-if = "this.role === 'CUSTOMER'" v-on:click = "showAllVehicles" class="btn btn-info"> See vehicles </button>
            <button v-if = "this.role === 'CUSTOMER'" v-on:click = "showAllRents" class="btn btn-info"> My orders </button>
            <button v-if = "this.role === 'CUSTOMER'" v-on:click = "showBasket" class="btn btn-info"> My basket </button>
            <button v-if = "this.role === 'MANAGER'" v-on:click = "showAllRents" class="btn btn-info"> My orders </button>
        ...
        <div id="so-div">
            <div>
                <form>
                    <div class="search-group">
                        ...
                        <div class="searchType-group">
                            <select v-model="searchFuel" class="form-select" aria-label="Default select example">
                            <option disabled selected value="">Fuel Type</option>
                              <option v-for="t in ftypes">{{t}}</option>
                            </select>
                        </div>
                ...
                <!--<form>
                    <div class="sort-group">
                        ...
                        <input type="text" v-model = "searchLocation" placeholder="Location" class="form-control"></input>
                        <input type="text" v-model = "searchFuelType" placeholder="FuelType" class="form-control"></input>
                        <input type="text" v-model = "searchRating" placeholder="Rating" class="form-control"></input>
            ...
            <div>
			<label for="openSwitch" id="switchLabel">Show only open facilities:</label>
			<label class="switch" >
				<input type="checkbox" id="openSwitch" v-model="isOpenChecked">
...
`
	...
	methods : {
		...
		searchForFacility: function() {
			...
			this.search = this.searchName + "," + this.searchType + "," + this.searchFuel + ","
	...
	/*	axios.get('rest/facilities/ftypes/')
			.then(response => {
				this.ftypes = response.data;
			});*/
    }
});